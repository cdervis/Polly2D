...
struct diyfp // f * 2^e
{
    ...
    constexpr diyfp(u64 f_, int e_) noexcept
        : f(f_)
        , e(e_)
    {
    }
    ...
    */
    static diyfp mul(const diyfp& x, const diyfp& y) noexcept
    {
        ...
        const u64 u_lo = x.f & 0xFFFFFFFFu;
        const u64 u_hi = x.f >> 32u;
        const u64 v_lo = y.f & 0xFFFFFFFFu;
        const u64 v_hi = y.f >> 32u;

        const u64 p0 = u_lo * v_lo;
        const u64 p1 = u_lo * v_hi;
        const u64 p2 = u_hi * v_lo;
        const u64 p3 = u_hi * v_hi;

        const u64 p0_hi = p0 >> 32u;
        const u64 p1_lo = p1 & 0xFFFFFFFFu;
        const u64 p1_hi = p1 >> 32u;
        const u64 p2_lo = p2 & 0xFFFFFFFFu;
        const u64 p2_hi = p2 >> 32u;

        u64 Q = p0_hi + p1_lo + p2_lo;
        ...
        Q += u64{1} << (64u - 32u - 1u); // round, ties up

        const u64 h = p3 + p2_hi + p1_hi + (Q >> 32u);
...
boundaries compute_boundaries(FloatType value)
{
    ...
    constexpr u64 kHiddenBit = u64{1} << (kPrecision - 1); // = 2^(p-1)

    using bits_type = typename std::conditional<kPrecision == 24, u32, u64>::type;

    const u64 bits = reinterpret_bits<bits_type>(value);
    const u64 E    = bits >> (kPrecision - 1);
...
inline cached_power get_cached_power_for_binary_exponent(int e)
{
    ...
    static constexpr Polly::Array<cached_power, 79> kCachedPowers = {{
...
*/
inline int find_largest_pow10(const u32 n, u32& pow10)
{
    // LCOV_EXCL_START
    if (n >= 1000000000)
    {
...
inline void grisu2_round(char* buf, int len, u64 dist, u64 delta, u64 rest, u64 ten_k)
{
    ...
    diyfp M_plus)
{
    ...
    u64 delta = diyfp::sub(M_plus, M_minus).f; // (significand of (M+ - M-), implicit exponent is e)
    u64 dist  = diyfp::sub(M_plus, w).f; // (significand of (M+ - w ), implicit exponent is e)
    ...
    while (n > 0)
    {
        ...
        //
        const u32 d = p1 / pow10; // d = p1 div 10^(n-1)
        const u32 r = p1 % pow10; // r = p1 mod 10^(n-1)
        //
        ...
        const u64 rest = (u64{p1} << -one.e) + p2;
        if (rest <= delta)
        {
            ...
            //
            const u64 ten_n = u64{pow10} << -one.e;
    ...
    for (;;)
    {
        ...
        //
        p2 *= 10;
        const u64 d = p2 >> -one.e; // d = (10 * p2) div 2^-e
        const u64 r = p2 & (one.f - 1); // r = (10 * p2) mod 2^-e
        //
...
@pre max_exp > 0
*/
inline char* format_buffer(char* buf, int len, int decimal_exponent, int min_exp, int max_exp, bool isInt)
{
    const int k = len;
...
char* to_chars(char* first, const char* last, double value, bool isInt)
{
    ...
    bool negative = Polly::sign(value);
    if (negative)
    {
        value    = -value;
        *first++ = '-';
    ...
    return dtoa_impl::format_buffer(first, len, decimal_exponent, kMinExp, kMaxExp, isInt);
}
} // namespace internal
} // namespace simdjson
