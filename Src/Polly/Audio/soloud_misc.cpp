...
float generateWaveform(int aWaveform, float p)
{
    ...
    {
        default:
        case WAVE_SQUARE: return p > 0.5f ? 0.5f : -0.5f;
        case WAVE_SAW: return p - 0.5f;
        case WAVE_SIN: return (float)sin(p * M_PI * 2.0f) * 0.5f;
        case WAVE_TRIANGLE: return (p > 0.5f ? (1.0f - (p - 0.5f) * 2) : p * 2.0f) - 0.5f;
        case WAVE_BOUNCE:
            return (p < 0.5f ? (float)sin(p * M_PI * 2.0f) * 0.5f : -(float)sin(p * M_PI * 2.0f) * 0.5f)
                   - 0.5f;
        case WAVE_JAWS: return (p < 0.25f ? (float)sin(p * M_PI * 2.0f) * 0.5f : 0) - 0.5f;
        case WAVE_HUMPS: return (p < 0.5f ? (float)sin(p * M_PI * 2.0f) * 0.5f : 0) - 0.5f;
        case WAVE_FSQUARE: {
            float f = 0;
