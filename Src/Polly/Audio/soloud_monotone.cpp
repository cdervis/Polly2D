...
unsigned int MonotoneInstance::getAudio(
    float*       aBuffer,
    ...
    unsigned int /*aBufferSize*/)
{
    ...
    {
        mOutput[i].mEnabled =
            i < (unsigned int)mParent->mHardwareChannels && i < (unsigned int)mParent->mSong.mTotalTracks;
    }
    for (i = 0; i < aSamplesToRead; i++)
    {
        if ((mSampleCount % samplesPerTick) == 0)
        {
            ...
            if (mRowTick >= mTempo)
            {
                ...
                {
                    unsigned int d =
                        mParent->mSong.mPatternData[(pattern * 64 + mRow) * mParent->mSong.mTotalTracks + j];
                    ...
                    switch (effect)
                    {
                        ...
                        case 0x4:
                            ...
                            if (effectdata2 != 0)
                                mChannel[j].mVibratoDepth = effectdata2;
                            if (effectdata1 != 0)
                                mChannel[j].mVibratoSpeed = effectdata1;
                            break;
                        case 0x5:
                            // pattern jump
            ...
            for (j = 0; j < mParent->mSong.mTotalTracks; j++)
            {
                if (mChannel[j].mActive)
                {
                    if (mChannel[j].mVibrato)
                    {
                        mChannel[j].mFreq[0] =
                            mParent->mNotesHz
                                [mChannel[j].mLastNote * 8
                                 + (mParent->mVibTable[mChannel[j].mVibratoIndex] * mChannel[j].mVibratoDepth)
                                       / 64];
            ...
            while (gotit < mParent->mHardwareChannels && tries < mParent->mSong.mTotalTracks)
            {
                if (mChannel[mNextChannel].mActive)
                {
                    ...
                    {
                        mOutput[gotit].mSamplePosInc =
                            1.0f
                            / (mSamplerate
                               / mChannel[mNextChannel].mFreq[mChannel[mNextChannel].mArpCounter]);
        ...
        for (j = 0; j < 12; j++)
        {
            if (mOutput[j].mEnabled)
            {
                ...
                aBuffer[i] +=
                    SoLoud::Misc::generateWaveform(mParent->mWaveform, mOutput[j].mSamplePos) * 0.5f;
            }
        }
