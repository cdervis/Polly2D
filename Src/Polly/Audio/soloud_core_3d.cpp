...
float attenuateExponentialDistance(
    float aDistance,
    float aMinDistance,
    float aMaxDistance,
...
void Soloud::update3dVoices_internal(unsigned int* aVoiceArray, unsigned int aVoiceCount)
{
    ...
    for (i = 0; i < (signed)aVoiceCount; i++)
    {
        ...
        {
            vol *= v->mAttenuator
                       ->attenuate(dist, v->m3dMinDistance, v->m3dMaxDistance, v->m3dAttenuationRolloff);
        }
        else
        {
            switch (v->m3dAttenuationModel)
            {
                ...
                case AudioSource::EXPONENTIAL_DISTANCE:
                    vol *= attenuateExponentialDistance(
                        dist,
                        v->m3dMinDistance,
                        v->m3dMaxDistance,
                        v->m3dAttenuationRolloff);
...
void Soloud::set3dListenerParameters(
    float aPosX,
    float aPosY,
    float aPosZ,
    float aAtX,
...
void Soloud::set3dSourceParameters(
    handle aVoiceHandle,
    float  aPosX,
    float  aPosY,
    float  aPosZ,
...
void Soloud::set3dSourceAttenuation(
    handle       aVoiceHandle,
