...
// #define FLOATING_POINT_DEBUG
    ...
    && !defined(WITH_SDL1)                                                                                   \
    && !defined(WITH_PORTAUDIO)                                                                              \
    && !defined(WITH_OPENAL)                                                                                 \
    && !defined(WITH_XAUDIO2)                                                                                \
    && !defined(WITH_WINMM)                                                                                  \
    && !defined(WITH_WASAPI)                                                                                 \
    && !defined(WITH_OSS)                                                                                    \
    && !defined(WITH_SDL1_STATIC)                                                                            \
    && !defined(WITH_SDL2_STATIC)                                                                            \
    && !defined(WITH_ALSA)                                                                                   \
    && !defined(WITH_OPENSLES)                                                                               \
    && !defined(WITH_NULL)                                                                                   \
    && !defined(WITH_COREAUDIO)                                                                              \
    && !defined(WITH_VITA_HOMEBREW)                                                                          \
    && !defined(WITH_JACK)                                                                                   \
    && !defined(WITH_NOSOUND)                                                                                \
...
result AlignedFloatBuffer::init(unsigned int aFloats)
{
    ...
    mBasePtr = static_cast<unsigned char*>(std::malloc(sizeof(unsigned char) * aFloats * sizeof(float)));
    if (mBasePtr == nullptr)
        return OUT_OF_MEMORY;
    mData = (float*)mBasePtr;
#else
    mBasePtr =
        static_cast<unsigned char*>(std::malloc((sizeof(unsigned char) * aFloats * sizeof(float)) + 16));
    if (mBasePtr == nullptr)
        return OUT_OF_MEMORY;
...
Soloud::Soloud()
{
    ...
    mMaxActiveVoices   = 16;
    mHighestVoice      = 0;
    mResampleData      = nullptr;
    mResampleDataOwner = nullptr;
...
void Soloud::deinit()
{
    assume(!mInsideAudioThreadMutex);
    stopAll();
...
void Soloud::clip_internal(
    AlignedFloatBuffer& aBuffer,
            ...
            while (step_fixed != 0 && outofs < aSamplesToRead)
            {
                if (voice->mLeftoverSamples == 0)
                {
                    ...
                    if (!voice->hasEnded() || voice->mFlags & AudioSourceInstance::LOOPING)
                    {
                        ...
                        if (readcount < SAMPLE_GRANULARITY)
                        {
                            if (voice->mFlags & AudioSourceInstance::LOOPING)
                            {
                                ...
                                {
                                    voice->mLoopCount++;
                                    int inc = voice->getAudio(
                                        voice->mResampleData[0]->mData + readcount,
                    ...
                    if (readcount < SAMPLE_GRANULARITY)
                    {
                        ...
                        for (k = 0; k < voice->mChannels; k++)
                            memset(
                                voice->mResampleData[0]->mData + readcount + SAMPLE_GRANULARITY * k,
                                0,
                ...
                {
                    writesamples =
                        ((SAMPLE_GRANULARITY * FIXPOINT_FRAC_MUL) - voice->mSrcOffset) / step_fixed + 1;
...
void Soloud::calcActiveVoices_internal()
{
    ...
    {
        if (mVoice[i]
            && (!(mVoice[i]->mFlags & (AudioSourceInstance::INAUDIBLE | AudioSourceInstance::PAUSED))
...
void Soloud::mix_internal(unsigned int aSamples)
{
    ...
    for (i = 0; i < FILTERS_PER_STREAM; i++)
    {
        ...
        {
            mFilterInstance[i]
                ->filter(mOutputScratch.mData, aSamples, mChannels, (float)mSamplerate, mStreamTime);
        }
    }
...
void deinterlace_samples_float(
    const float* aSourceBuffer,
    float*       aDestBuffer,
    unsigned int aSamples,
    unsigned int aChannels)
{
