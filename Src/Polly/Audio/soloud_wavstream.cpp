...
WavStreamInstance::WavStreamInstance(WavStream* aParent)
{
    ...
    if (mFile)
    {
        ...
        {
            assumeWithMsg(false, "not implemented");
#if 0 // TODO:
				int e;
        ...
        {
            mCodec.mFlac = drflac_open(drflac_read_func, drflac_seek_func, nullptr, mFile, nullptr);
            if (!mCodec.mFlac)
            {
                if (mFile != mParent->mStreamFile)
                    delete mFile;
    ...
    int     aChannels)
{
    ...
    {
        std::memcpy(aBuffer + aPitch * i, aOggOutputs[i] + aFrameOffset, sizeof(float) * samples);
    }
    return samples;
}


unsigned int WavStreamInstance::getAudio(
    float*       aBuffer,
...
result WavStream::loadwav(File* fp)
{
    ...
    if (!drwav_init(&decoder, drwav_read_func, drwav_seek_func, nullptr, fp, nullptr))
...
{
    fp->seek(0);
    drflac* decoder = drflac_open(drflac_read_func, drflac_seek_func, nullptr, fp, NULL);
...
result WavStream::load(const char* aFilename)
{
    ...
    mFilename = static_cast<char*>(std::malloc(sizeof(char) * (len + 1)));
    std::memcpy(mFilename, aFilename, len);
    mFilename[len] = 0;
...
result WavStream::loadMem(const unsigned char* aData, unsigned int aDataLen, bool aCopy, bool aTakeOwnership)
{
    ...
    auto* mf  = new MemoryFile(MemoryFile());
