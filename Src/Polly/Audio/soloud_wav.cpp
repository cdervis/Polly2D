...
result Wav::loadwav(MemoryFile* aReader)
{
    ...
    mData = static_cast<float*>(std::malloc(sizeof(float) * (unsigned int)(samples * decoder.channels)));
    mBaseSamplerate = (float)decoder.sampleRate;
    mSampleCount    = (unsigned int)samples;
    mChannels       = decoder.channels;

    auto tmp = static_cast<float*>(std::malloc(sizeof(float) * 512 * MAX_CHANNELS));
...
result Wav::loadogg(MemoryFile* aReader)
{
    ...
    mData        = static_cast<float*>(std::malloc(sizeof(float) * samples * mChannels));
    ...
    while (1)
    {
        ...
        {
            std::memcpy(mData + samples + mSampleCount * ch, outputs[ch], sizeof(float) * n);
        }
...
result Wav::loadRawWave8(unsigned char* aMem, unsigned int aLength, float aSamplerate, unsigned int aChannels)
{
    ...
    stop();
    std::free(mData);
    mData           = static_cast<float*>(std::malloc(sizeof(float) * aLength));
    ...
    bool         aTakeOwndership)
{
    ...
    {
        mData = static_cast<float*>(std::malloc(sizeof(float) * aLength));
        std::memcpy(mData, aMem, sizeof(float) * aLength);
    }
    else
    {
        mData = aMem;
