...
static float DBtoLIN(int dB)
{
    ...
    static const float amptable[88] = {
        ...
        0.0,     0.0,     6.0,     7.0,     8.0,     9.0,     10.0,    11.0,    13.0,    14.0,    16.0,
        18.0,    20.0,    22.0,    25.0,    28.0,    32.0,    35.0,    40.0,    45.0,    51.0,    57.0,
        64.0,    71.0,    80.0,    90.0,    101.0,   114.0,   128.0,   142.0,   159.0,   179.0,   202.0,
        227.0,   256.0,   284.0,   318.0,   359.0,   405.0,   455.0,   512.0,   568.0,   638.0,   719.0,
        811.0,   911.0,   1024.0,  1137.0,  1276.0,  1438.0,  1622.0,  1823.0,  2048.0,  2273.0,  2552.0,
        2875.0,  3244.0,  3645.0,  4096.0,  4547.0,  5104.0,  5751.0,  6488.0,  7291.0,  8192.0,  9093.0,
        10207.0, 11502.0, 12976.0, 14582.0, 16384.0, 18350.0, 20644.0, 23429.0, 26214.0, 29491.0, 32767.0};
...
float klatt::natural_source(int aNper)
{
    ...
    if (aNper < mNOpen)
    {
        ...
        {
            case KW_TRIANGLE: return ((aNper % 200) - 100) * 81.92f; // triangle
            case KW_SIN: return (float)(sin(aNper * 0.0314) * 8192); // sin
            case KW_SQUARE: return ((aNper % 200) - 100) > 0 ? 8192.0f : -8192.0f; // square
            case KW_PULSE: return ((aNper % 200) - 100) > 50 ? 8192.0f : -8192.0f; // pulse
            case KW_NOISE: return (int)mNLast & 1 ? -8192.0f : 8192.0f;
            case KW_WARBLE: return (int)mNLast & 7 ? -8192.0f : 8192.0f;
            case KW_SAW: // fallthrough
            default: return (abs((aNper % 200) - 100) - 50) * 163.84f; // saw
        }
    }
    else
    {
...
void klatt::pitch_synch_par_reset(int ns)
{
    if (mF0FundamentalFreq > 0)
    {
        ...
        if (mSkew >= 0)
        {
            mSkew = mSkewnessOfAlternatePeriods; /* Reset mSkew to requested
    ...
    for (ns = 0; ns < mNspFr; ns++)
    {
        ...
        */
        ...
        for (n4 = 0; n4 < 4; n4++)
        {
            ...
            /* to globals->mSampleRate samples/sec.  Resonator f=.09*globals->mSampleRate,
             * bw=.06*globals->mSampleRate  */
...
int klatt::synth(int /* aSampleCount */, short* aSamplePointer)
{
    ...
    if (dur > 0)
    {
        ...
        for (t = 0; t < dur; t++, mTStress++)
        {
            ...
            {
                tp[j] =
                    interpolate(&start[j], &end[j], (float)currentElement->mInterpolator[j].mSteady, t, dur);
            }
            ...
            mFrame.mF0FundamentalFreq =
                (int)(base
                      + (mTop - base) * interpolate(&mStressS, &mStressE, (float)0, mTStress, mNTStress));
