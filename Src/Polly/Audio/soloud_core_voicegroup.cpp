...
handle Soloud::createVoiceGroup()
{
    ...
    for (i = 0; i < mVoiceGroupCount; i++)
    {
        ...
        {
            mVoiceGroup[i] = static_cast<unsigned int*>(std::malloc(sizeof(unsigned int) * 16));
            if (mVoiceGroup[i] == nullptr)
            {
    ...
    mVoiceGroupCount *= 2;
    unsigned int** vg = static_cast<unsigned int**>(std::malloc(sizeof(unsigned int*) * mVoiceGroupCount));
    if (vg == nullptr)
    {
    ...
    mVoiceGroup    = vg;
    i              = oldcount;
    mVoiceGroup[i] = static_cast<unsigned int*>(std::malloc(sizeof(unsigned int) * 17));
    if (mVoiceGroup[i] == nullptr)
    {
        unlockAudioMutex_internal();
        return 0;
...
result Soloud::addVoiceToGroup(handle aVoiceGroupHandle, handle aVoiceHandle)
{
    ...
    unsigned int* n =
        static_cast<unsigned int*>(std::malloc(sizeof(unsigned int) * (mVoiceGroup[c][0] * 2 + 1)));
    if (n == nullptr)
    {
        unlockAudioMutex_internal();
