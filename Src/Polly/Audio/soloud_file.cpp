...
unsigned int MemoryFile::read(unsigned char* aDst, unsigned int aBytes)
{
    ...
    std::memcpy(aDst, mDataPtr + mOffset, aBytes);
    mOffset += aBytes;
    ...
    bool                 aTakeOwnership)
{
    ...
    {
        mDataOwned = true;
        mDataPtr   = static_cast<unsigned char*>(std::malloc(sizeof(unsigned char) * aDataLength));
        if (mDataPtr == nullptr)
            return OUT_OF_MEMORY;
        std::memcpy((void*)mDataPtr, aData, aDataLength);
        return SO_NO_ERROR;
    }

    mDataPtr   = const_cast<unsigned char*>(aData);
...
result MemoryFile::openToMem(const char* aFile)
{
    ...
    mDataPtr    = static_cast<unsigned char*>(std::malloc(sizeof(unsigned char) * mDataLength));
    if (mDataPtr == nullptr)
        return OUT_OF_MEMORY;
